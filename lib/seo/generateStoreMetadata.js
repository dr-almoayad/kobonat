 ...
 *
 * ✅ FIX: generateEnhancedStoreMetadata now accepts explicit arSlug/enSlug
 * params so hreflang alternate URLs use the correct locale-specific slug
 * rather than the same slug for both languages. Without this, Arabic store
 * pages (which have Arabic slugs) produce broken en-SA hreflang URLs that
 * Google treats as 404s — causing the entire hreflang set to be ignored
 * and flagging pages as "canonicalized" in crawlers.
 */
...
export function generateEnhancedStoreMetadata({
  ...
  categories = [],
  country,
  arSlug,  // ← explicit Arabic slug (pass from page.jsx)
  enSlug,  // ← explicit English slug (pass from page.jsx)
}) {
  const [language, countryCode] = locale.split('-');
  const isArabic = language === 'ar';

  // Use the current slug from the store object as the canonical slug.
  // arSlug/enSlug are used only for hreflang alternates.
  ...
  return {
    ...
    // ✅ FIX: Use locale-specific slugs for hreflang so ar-SA and en-SA
    // alternate URLs resolve to real pages rather than redirecting/404ing.
    alternates: {
...
// ✅ NOTE: generateHreflangAlternates is kept for any other callers but is no
// longer used internally — generateEnhancedStoreMetadata builds alternates
// inline using the resolved arSlug/enSlug params.
...
export function generateEnhancedCategoryMetadata({
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        'ar-SA': `${BASE_URL}/ar-SA/stores/${category.slug}`,
        'en-SA': `${BASE_URL}/en-SA/stores/${category.slug}`,
        'x-default': `${BASE_URL}/ar-SA/stores/${category.slug}`,
      },
    },
    openGraph: {
      type: 'website',
