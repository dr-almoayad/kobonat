 ...
 *
 * FIX 2 (categories): generateEnhancedCategoryMetadata now uses /categories/ in
 *   canonical and alternate URLs, not /stores/. The function also accepts explicit
 *   arSlug/enSlug for correct per-locale hreflang generation.
 */
...
export function generateEnhancedCategoryMetadata({
  ...
  // ✅ FIX: Use /categories/ instead of /stores/
  ...
  const defaultPath = `/ar-SA/categories/${resolvedArSlug}`;
