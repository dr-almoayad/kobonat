...
export function generateEnhancedStoreMetadata({
  ...
  // ✅ FIX: Passing localized country name instead of raw 'SA'
...
// ✅ FIX: Removed rotating month/year to eliminate false freshness signals
...
function generateKeywords(store, categories, isArabic, countryName) {
  ...
  if (isArabic) {
    return [
      ...
      `${storeName} ${countryName}`, `كوبون ${storeName}`, `برومو كود ${storeName}`, // ✅ Uses countryName instead of 'SA'
...
export function generateEnhancedCategoryMetadata({
  ...
  // ✅ FIX: Removed rotating 'year' to eliminate false freshness signals
