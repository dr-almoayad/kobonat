...
// No month/year or "daily" strings to avoid false freshness signals.
...
export function generateStorePageTitle({
  ...
  // ✅ FIX: Removed "confirmed daily" and "updated regularly" to maintain evergreen accuracy
  const description = count && savings
    ? `Get ${count}+ verified ${storeName} discount codes. Save ${savings}% on your purchases — tested and confirmed.`
    : count
      ? `Get ${count}+ verified ${storeName} coupon codes. Exclusive deals tested and confirmed.`
      : savings
        ? `Latest ${storeName} coupon codes with up to ${savings}% off — verified discounts and exclusive deals.`
        : `Latest ${storeName} coupon codes — verified discounts, free shipping offers and exclusive deals.`;
 ...
 * Stable – no false freshness signals.
 */
export function generateStoreHeroSubtitle({
  ...
  if (isAr) {
    ...
    // ✅ FIX: Removed unconditional 'محدّث يومياً'
    return parts.join(' • ');
  }
  ...
  // ✅ FIX: Removed unconditional 'Updated daily'
  return parts.join(' • ');
}
