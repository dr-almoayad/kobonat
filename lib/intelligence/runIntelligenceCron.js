...
async function computeOfferQualityRatio(storeId, now) {
  ...
  const [total, qualityCount] = await Promise.all([
    // Total active offers (vouchers + promos)
    prisma.voucher.count({ where: activeWhere }),

    // Offers that are verified OR exclusive (deduplicated with distinct count)
    prisma.voucher.count({
      where: {
        ...activeWhere,
        OR: [{ isVerified: true }, { isExclusive: true }],
      },
    }),
  ]);

  if (total === 0) return 0;
  return qualityCount / total; // 0.0–1.0
}
...
export async function runIntelligenceCron() {
