  ...
  try {
    ...
    const data = stores.map((store) => {
      ...
      return {
        ...
        pageUrlAr: ar.slug ? `${BASE_URL}/ar-SA/stores/${ar.slug}` : null,
        pageUrlEn: en.slug ? `${BASE_URL}/en-SA/stores/${en.slug}` : null,
    ...
    console.error('[GET /api/feeds/stores.json]', error);
    return NextResponse.json(
      { error: 'Failed to generate stores feed', generatedAt: new Date().toISOString(), stores: [] },
      { status: 503 }
    );
  }
}
