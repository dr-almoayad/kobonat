...
export async function generateMetadata({ params, searchParams }) {
  ...
  return {
    ...
    alternates: {
      ...
      languages: { 'ar-SA': `${baseUrl}/ar-SA/blog`, 'en-SA': `${baseUrl}/en-SA/blog` },
    },
    openGraph: {
...
async function getPosts({ locale, categorySlug, tagSlug, page, limit }) {
  ...
  // ── BUG FIX ────────────────────────────────────────────────────────────────
  // The old filter included `publishedAt: { lte: new Date() }`.
  // Prisma evaluates NULL <= now() as FALSE, so every post published without
  // an explicit date was silently excluded even when status = PUBLISHED.
  // Removed — status: 'PUBLISHED' is the only gate we need.
  // ──────────────────────────────────────────────────────────────────────────
...
async function getCategories(lang) {
  try {
    return await prisma.blogCategory.findMany({
      include: {
        translations: { where: { locale: lang } },
        // Count only PUBLISHED posts — no publishedAt filter here either
      ...
      <main dir={isRTL ? 'rtl' : 'ltr'} style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 16px' }}>
        ...
        {transformedPosts.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '80px 20px', color: '#888' }}>
            ...
            <Link href={`/${locale}/blog`} style={{ color: '#470ae2' }}>{labels.allPosts}</Link>
          </div>
        ) : (
          <div style={{
            display: 'grid',
        ...
        {pages > 1 && (
          ...
          >
            {page > 1 && (
              <PaginationLink href={buildPageUrl(locale, category, tag, page - 1)} label={labels.prev} isRTL={isRTL} />
            )}

            {getPaginationRange(page, pages).map((p, i) =>
              p === '...' ? (
                <span key={`e-${i}`} style={{ padding: '8px 4px', color: '#aaa' }}>…</span>
              ) : (
                <PaginationLink
                  key={p}
            ...
            {page < pages && (
              <PaginationLink href={buildPageUrl(locale, category, tag, page + 1)} label={labels.next} isRTL={isRTL} />
            )}
          </nav>
        )}
  ...
  return (
    <Link
      ...
      style={{
        ...
        fontSize: '0.875rem', fontWeight: isActive ? 700 : 400, textDecoration: 'none',
